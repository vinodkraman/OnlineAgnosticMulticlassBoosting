...
import numpy as np
import copy
from algorithms.onlineGradientDescentRealizable import OnlineGradientDescent, projection_simplex_sort
...
class OCORealizableBoost:
    ...
    def update(self, Y, X=None, verbose=False):
        ...
        for i in range(self.num_wls):
            ...
            yes = np.random.binomial(1, p, 1)
            if yes:
                self.weaklearners[i].learn_one(self.X_dict, self.Y_index)
