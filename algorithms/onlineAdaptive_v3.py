...
class AdaBoostOLM:
    ...
    def compute_cost_v2(self, s, i, y):
        k = self.num_classes
        ret = np.zeros(k)
        r  = y
        for l in range(k):
            e = np.zeros(k)
            e[l] = 1
            ret[l] = self.get_potential(r, self.num_wls-i-1, s+e) #why -1: oh makes sense
        return ret 
    ...
    def get_weight_v2(self, cost_vec, i):
        if self.loss == 'logistic':
            if i == 0:
                return 5
            const = self.weight_consts[i]
            ret = -const * cost_vec[self.Y_index]/(self.num_classes-1)
        elif self.loss == 'exp':
            ret = -cost_vec[self.Y_index]/(self.num_classes-1)
            N = np.exp(0.2*np.sqrt(i))
            ret = 10 * ret / N
        else:
            const = self.weight_consts[i]
    ...
    def predict(self, X, verbose=False):
        ...
        for i in range(self.num_wls):
            ...
            wl_preds[i] = np.argmax(pred_probs)
            # if verbose is True:
    ...
    def update(self, Y, X=None, verbose=False):
        ...
        expert_votes = np.zeros(self.num_classes)
        # cost_vec = self.compute_cost_v2(expert_votes, 0, self.Y_index)
        for i in range(self.num_wls):
            # cost_vec = self.compute_cost_v2(expert_votes, i, self.Y_index)
            alpha = self.wl_weights[i]

            w = self.get_weight(i) 
            self.weaklearners[i].learn_one(self.X_dict, self.Y_index, sample_weight = float(w))
            ...
            # cost_vec = self.compute_cost_v2(expert_votes, i+1, self.Y_index)
    ...
    def initialize_dataset(self, filename, class_index, probe_instances=10000):
        ...
        print("realizable", self.num_classes)
    ...
    def gen_weaklearners(self, num_wls, leaf_pred = "nba", max_depth=None, min_weight = 10, max_weight = 200):
