...
class OnlineRealizableBoost:
    ...
    def update(self, Y, X=None, verbose=False):
        ...
        for i in range(self.num_wls):
            ...
            # for jndex in range(len(p)):
            #     self.weaklearners[i].learn_one(self.X_dict, jndex, sample_weight= float(p[jndex]))
    ...
    def initialize_dataset(self, filename, class_index, probe_instances=10000):
        ...
        self.oco = OnlineGradientDescent(1)
    ...
    def gen_weaklearners(self, num_wls, min_conf = 0.00001, max_conf = 0.9, 
        ...
        self.weaklearners = [tree.HoeffdingTreeClassifier(leaf_prediction= "mc") for i in range(num_wls)]
        # self.weaklearners = [VeryFastDecisionRulesClassifier(nb_prediction=False) for i in range(num_wls)]
