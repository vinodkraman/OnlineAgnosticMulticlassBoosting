...
class AdaBoostOLM:
    ...
    def predict(self, X, verbose=False):
        ...
        for i in range(self.num_wls):
            ...
            # pred_probs = self.weaklearners[i].predict_proba(self.X)
            # pred_probs = pred_probs[0]

            # if np.sum(pred_probs) == 0:
            #     pred_probs = np.ones(self.num_classes) * 1.0/float(self.num_classes)
            ...
            expected_costs = np.dot(out, cost_mat) 
            print(self.X_dict)
            print(cost_mat)
            print(out)
            ...
            if len(tmp) == 1 and len(np.unique(out)) == 1:
                print("hello", tmp[0], np.argmax(out))
            wl_preds[i] = np.random.choice(tmp)
            if verbose is True:
                print (i, expected_costs, out, wl_preds[i])
    ...
    def update(self, Y, X=None, verbose=False):
        ...
        for i in range(self.num_wls):
            ...
            # y_array = np.array([self.Y_index])
            # w_array = np.array([float(w)])
            # self.weaklearners[i].partial_fit(self.X, y_array, classes= np.arange(self.num_classes), sample_weight= w_array)
    ...
    def gen_weaklearners(self, num_wls, min_conf = 0.00001, max_conf = 0.9, 
        ...
        self.weaklearners = [tree.HoeffdingTreeClassifier() for i in range(num_wls)]
