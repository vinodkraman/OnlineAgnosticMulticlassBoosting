...
for index in tqdm(range(num_exp)):
    ...
    for gamma_index in range(len(gammas_R)):
        ...
        if which_model == "all" or which_model == "OR":
            ...
            wl_perf_ROCO.append((ov_result_ROCO, result_ROCO, gammas_A[gamma_index]))
            end = timer()
        ...
        if which_model == "all" or which_model == "A":
            ...
            wl_perf_A.append((ov_result_A, result_A, gammas_A[gamma_index]))
            end = timer()
            print("Agnostic complete", end- start)

        if which_model == "all" or which_model == "R":
            start = timer()
            model = AdaBoostOLM(num_classes, loss='logistic', gamma=gammas_R[gamma_index], nom_att=nom_att)
            model.M = 100
            model.gen_weaklearners(num_wl, leaf_pred= leaf_pred, max_depth = max_depth) 
            result_R, ov_result_R = run_boosting(model, train_rows, test_rows)
            wl_perf_R.append((ov_result_R, result_R, gammas_R[gamma_index]))
            end = timer()
