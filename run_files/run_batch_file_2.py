...
# for index in tqdm(range(num_splits)):
for index in range(num_splits):
    ...
    for gamma_index in range(len(gammas_A)):
        for m0_frac in m0_values:
            ...
            y_pred = model.predict(X_test, random=False)
            accuracy = accuracy_score(y_test, y_pred)
            agnostic_run_perf.append((accuracy, gammas_A[gamma_index], m0_frac))
            print((accuracy, gammas_A[gamma_index], m0_frac))
        ...
        accuracy = accuracy_score(y_test, y_pred)
        real_run_perf.append((accuracy, gammas_R[gamma_index]))

    print("Agnostic", index, agnostic_run_perf)
